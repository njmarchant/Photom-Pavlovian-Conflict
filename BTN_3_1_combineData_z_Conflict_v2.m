...
CSP = []; CSM = []; CSPun = []; % Initialize data matrices
CSP_source = {}; CSM_source = {}; CSPun_source = {}; % Initialize cell arrays to track data origin
...
for k = 1:length(files)
    ...
    if exist('sesdat', 'var') && isfield(sesdat, 'traces_z') && ~isempty(sesdat.traces_z)
        ...
        cspun_orig_rows = all_rows(sesdat.traces_z(:, 2) == 3);
        
        % Append the trace data and its source information
        if ~isempty(csp_orig_rows)
            CSP = [CSP; sesdat.traces_z(csp_orig_rows, 3:end)];
        ...
        if ~isempty(csm_orig_rows)
            CSM = [CSM; sesdat.traces_z(csm_orig_rows, 3:end)];
            for r = 1:length(csm_orig_rows)
                CSM_source{end+1, 1} = sprintf('%s, original row %d', files(k).name, csm_orig_rows(r));
            end
        end
        if ~isempty(cspun_orig_rows)
            CSPun = [CSPun; sesdat.traces_z(cspun_orig_rows, 3:end)];
            for r = 1:length(cspun_orig_rows)
                CSPun_source{end+1, 1} = sprintf('%s, original row %d', files(k).name, cspun_orig_rows(r));
            end
        end
...
mean_cspun = mean(CSPun, 2);
outlier_idx_cspun = mean_cspun > 20 | mean_cspun < -20;
if any(outlier_idx_cspun)
    fprintf('Found and removed %d rows from CSPun with mean outside +/- 20.\n', sum(outlier_idx_cspun));
    CSPun(outlier_idx_cspun, :) = [];
    CSPun_source(outlier_idx_cspun) = [];
end
